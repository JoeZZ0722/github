...
# load the dataset
dataframe = pandas.read_excel('data00.xlsx', usecols=[1], sep = ',')
...
model = Sequential()
model.add(LSTM(6,input_shape=(1,31),return_sequences=True))
model.add(LSTM(16))
model.add(Dense(31))
...
df2 = pandas.read_excel('data00.xlsx',sep=',',index_col='Date')
df3 = pandas.concat([df2,df1],ignore_index= False)
df3.to_excel('data0012.xlsx')
...
rng2 = pandas.date_range('12/01/2017', periods=14, freq='D')
df4 = pandas.DataFrame(Relative_error_rate,index=rng2,columns=['LSTM'])
df4.to_excel('lstm00.xlsx')
