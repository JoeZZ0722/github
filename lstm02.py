...
# load the dataset
dataframe = pandas.read_excel('data0501.xlsx', usecols=[0], sep = ',')
dataset1 = dataframe.values[:-31,]
dataset2 = dataframe.values[-31:,]
...
print feedXX
print Predict[0]
print dataframe.values[-31:,0]

dataset3 = Predict[0]
rng = pandas.date_range('02/01/2018', periods=len(dataset3), freq='D')
df1 = pandas.DataFrame(dataset3,index=rng,columns=['DataflowByDay_4G_GB'])
df2 = pandas.read_excel('data0501.xlsx',sep=',')
df3 = pandas.concat([df2,df1],ignore_index= False)
df3.to_excel('data0502.xlsx')
